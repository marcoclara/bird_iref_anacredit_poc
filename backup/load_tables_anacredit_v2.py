...
for file_name in os.listdir(input_dir):
    ...
    for sheet_name in xls.sheet_names:
        entidade_normalizada = normalizar_entidade(sheet_name)
        tabela = entidade_para_tabela.get(entidade_normalizada)
        atributos_validos = atributos_por_tabela.get(tabela)

        if not tabela or not atributos_validos:
            print(f"⚠️ Sheet ignorada (sem mapeamento): {sheet_name} → {entidade_normalizada}")
            continue
        ...
        # Substituir "Non-applicable" por None (NULL)
